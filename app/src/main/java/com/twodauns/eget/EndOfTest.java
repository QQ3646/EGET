...
import com.twodauns.eget.check.CheckResult;
import com.twodauns.eget.check.InformaticsKt;
...
import static com.twodauns.eget.tester.notC;
...
public class EndOfTest extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Итоги");
        ...
        if (MainActivity.contentDescription.equals("История")) {
            line = 4;
            int i = 0;
            for (Task Task : InformaticsKt.checkINFO().getTasks()) {
                if (Task != null) {
...
//            }
            ...
            line = 10;
            checkResult = InformaticsKt.checkINFO();
...
//            }
        ...
        int i = 0;
        for (Task Task : checkResult.getTasks()) {
            if (Task != null) {
                TableRow tableRow = new TableRow(this);
                tableRow.addView(Task.getNumber());
                tableRow.addView(Task.getCorrectOrNot());
    ...
    public void setCButtonAction(Task Task) {
        Task.getShow().setOnClickListener(view -> {
            Intent intent = new Intent(ctn, EndOfTestViewer.class);
            intent.putExtra("Points", tester.answers[Task.getI()]);
